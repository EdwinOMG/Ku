...
interface Notification {
  id: string; type: 'like' | 'comment' | 'follow'; read: boolean; created_at: string
  actor: { username: string; avatar_url?: string }
  ku?: { line1: string; line2: string; line3: string }; ku_id?: string
}
...
const LABELS = { like: 'liked your ku', comment: 'commented on your ku', follow: 'followed you' }
        ...
        <div className="stagger">
          {notifications.map(n => (
            <button key={n.id} onClick={() => handleClick(n)}
              className={`flex items-start gap-3 px-4 py-3.5 border-b border-cafe-border/50 text-left w-full transition-all duration-200 hover:bg-cafe-warm animate-fade-in-up ${
